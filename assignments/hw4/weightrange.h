...
class WeightRange {
 public:
  ...
  Weight GetSmallest() const { return low_; }
  // Add an accessor function to return the largest weight for the range
  Weight GetLargest() const { return high_; }
  ...
  WeightRange& SetSmallest(const Weight& s);
  ...
  WeightRange& SetLargest(const Weight& l);
  ...
  bool InRange(const Weight& w, bool b = true);
